...
use super::{array_parser, control_flow_parser, function_parser, literal_parser, operator};
...
pub(super) fn parse_primary_expression_with_count(
    tokens: &[lexing::Token],
    ...
    match &tokens[0].token_type {
        ...
        lexing::TokenType::IntegerLiteral(num) => Ok((Expr::Number(*num as f64), 1)),
        lexing::TokenType::StringLiteral(value) => Ok((Expr::StringLiteral(value.clone()), 1)),
        lexing::TokenType::StringTemplate(_) => {
            // String templates consume 1 token
            let expr = literal_parser::parse_string_template(tokens)?;
            Ok((expr, 1))
        }
        lexing::TokenType::TrueLiteral(_) => Ok((Expr::Bool(true), 1)),
        lexing::TokenType::FalseLiteral(_) => Ok((Expr::Bool(false), 1)),
        _ if tokens.len() == 1
            && matches!(&tokens[0].token_type, lexing::TokenType::Identifier(id) if id.to_lowercase() == "null") =>
        {
            Ok((Expr::Null, 1))
        }
        ...
        lexing::TokenType::Identifier(value) => {
            ...
            if tokens.len() > 1
                && matches!(&tokens[1].token_type, lexing::TokenType::LeftBracket(_))
            {
            ...
            if tokens.len() > 2
                && matches!(&tokens[1].token_type, lexing::TokenType::Dot(_))
        ...
        lexing::TokenType::LeftParen(_) => parse_parenthesized_expression(tokens),

        // Unary expressions
        lexing::TokenType::Plus(_) | lexing::TokenType::Minus(_) => parse_unary_expression(tokens),
...
fn parse_index_access(tokens: &[lexing::Token]) -> ParseResult<Expr> {
    ...
    if tokens.len() < 3 {
        ...
        return Err(ParseError::invalid_expression(
            "Index access requires target[index] syntax",
            pos,
        ));
    }
...
pub(super) fn parse_comma_separated_expressions(
    ...
    for i in 0..=tokens.len() {
        if i == tokens.len()
            || (i < tokens.len()
                && matches!(&tokens[i].token_type, lexing::TokenType::Comma(_))
                && paren_count == 0
                && bracket_count == 0
                && brace_count == 0)
        {
        ...
        if i < tokens.len() {
            match &tokens[i].token_type {
                ...
                lexing::TokenType::LeftBracket(_) => bracket_count += 1,
                lexing::TokenType::RightBracket(_) => {
